...
namespace Polaris.UI
{
    ...
    public sealed class MainMenuPuiFacade
    {
            ...
            string hint = null)
        {
            ...
            if (UIAPI.Pui.TryGet(puiOrGraphName, out PUIRuntime pui))
            {
                AddButton(name, pui, insertIndex, submitLabel, onSubmit, cancelLabel, onCancel, hint);
                return;
            }

            if (UIAPI.Pui.TryGetGraph(puiOrGraphName, out _))
            {
                AddButton(name, UIAPI.Pui.GetDefaultSolution(puiOrGraphName), insertIndex, submitLabel, onSubmit, cancelLabel, onCancel, hint);
                return;
            }
        ...
        public PUIRuntime AddButton(string name, IPUI pui, int insertIndex = -1)
        {
            ...
            PUIRuntime runtime = UIAPI.Pui.IsRegistered(pui.Name)
                ? UIAPI.Pui.Get(pui.Name)
                : UIAPI.Pui.Register(pui);
