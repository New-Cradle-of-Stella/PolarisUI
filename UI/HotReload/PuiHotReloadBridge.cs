...
namespace Polaris.UI.HotReload
{
    ...
    internal static class PuiHotReloadBridge
    {
        ...
        private static IPuiCustomControl ResolveCustomControl(string typeName, IPUI handler)
        {
            ...
            {
                throw new InvalidOperationException(
                    $"Custom control backend type \"{typeName}\" does not implement Polaris.UI.IPuiCustomControl.");
            }
        ...
        private static MImage ResolveImage(string imageSource, IPUI handler)
        {
            ...
            return Polaris.Res.ResAPI.For(modId).Own.Image(imageSource);
        }
